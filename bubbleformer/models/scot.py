"""
This file contains scOT. All credit to the original authors of poseidon
"""
...
@register_model("scot")
class ScOTSpatiotemporal(nn.Module):
    """
    AViT-style wrapper around the original ScOT model.

    Args:
        input_fields (int):  number of input channels (C_in)
        output_fields (int): number of output channels (C_out)
        time_window (int):   number of time steps (B, T, C, H, W)  [kept for API compat; not used internally]
        image_size (int):    base spatial resolution for ScOT (assumes square, H == W)
        patch_size (int):    ScOT patch size
        embed_dim (int):     base embed dim of ScOT (default 96 as in SwinV2-Small)
        depths (tuple[int]): encoder/decoder depths per stage
        num_heads (tuple[int]): attention heads per stage
        drop_path (float):   max stochastic depth rate
        residual_model (str): "convnext" or "resnet" (for skip processing)
        use_conditioning (bool): whether to use the scalar `time` conditioning path
        learn_residual (bool): use residual learning (prediction + input)
        p (int):             1 (L1) or 2 (L2) for the built-in loss (unused in wrapper forward)
        channel_slice_list_normalized_loss (list[int] or None): same as original ScOT
    """
    ...
    def __init__(
        ...
        input_fields: int = 3,
        output_fields: int = 3,
        time_window: int = 12,        # kept for interface compatibility, not used internally
        image_size: int = 224,
        patch_size: int = 4,
        embed_dim: int = 96,
        ...
        # ---- Build ScOTConfig from AViT-style arguments ----
        config = ScOTConfig(
            image_size=image_size,
            patch_size=patch_size,
            num_channels=input_fields,
            num_out_channels=output_fields,
            embed_dim=embed_dim,
            depths=list(depths),
            ...
            skip_connections=[1] * len(depths),
        )

        # this is the original ScOT model you pasted above
        self.scot = ScOT(config, use_mask_token=False)

        # record a few things for convenience
        self.patch_size = patch_size
        self.image_size = image_size
    ...
    def forward(
        ...
        Args:
            x:      (B, T, C_in, H, W)
            time_cond: optional scalar conditioning per (B, T) or per (B*T,)
                       only used if use_conditioning=True in __init__.
            labels: optional (B, T, C_out, H, W), passed through to ScOT for loss
                    (you can ignore this and compute loss outside if you prefer).

        Returns:
            y: (B, T, C_out, H, W)
        """
        b, t, c, H, W = x.shape
        assert c == self.input_fields, f"Expected {self.input_fields} input channels, got {c}"

        # Flatten time into batch for ScOT: (B*T, C, H, W)
        x_img = rearrange(x, "b t c h w -> (b t) c h w")

        # Handle conditioning: make it (B*T,) if enabled
        if self.use_conditioning:
            if time_cond is None:
                raise ValueError("use_conditioning=True but time_cond is None")
            # allow (B,T), (B,T,1), or (B*T,)
            if time_cond.dim() == 2:     # (B, T)
                time_flat = rearrange(time_cond, "b t -> (b t)")
            elif time_cond.dim() == 3:   # (B, T, 1)
                time_flat = rearrange(time_cond, "b t 1 -> (b t)")
            elif time_cond.dim() == 1:   # (B*T,)
                assert time_cond.numel() == b * t
                time_flat = time_cond
            else:
                raise ValueError("time_cond must be of shape (B,T), (B,T,1) or (B*T,)")

            time_flat = time_flat.to(x_img.device, dtype=x_img.dtype)
        else:
            # ScOT expects a `time` argument everywhere, even if unused; give zeros
            time_flat = torch.zeros(b * t, device=x_img.device, dtype=x_img.dtype)

        # Optional labels path (same flattening as x)
        if labels is not None:
            assert labels.shape[0] == b and labels.shape[1] == t
            labels_img = rearrange(labels, "b t c h w -> (b t) c h w")
        else:
            labels_img = None
